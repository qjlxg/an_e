...
# 加载大盘数据（为保证脚本可运行，此处简化处理 FileNotFoundError）
try:
...
for code, cost_nav in holdings_config.items():
    ...
    if os.path.exists(fund_file):
        ...
        dates = pd.date_range(end=datetime.now(), periods=100, freq='D')
        # 模拟一个有峰值和回撤的数据
        net_values = np.linspace(cost_nav, cost_nav * 1.3, 80)
        net_values = np.append(net_values, np.linspace(cost_nav * 1.3, cost_nav * 1.2, 20))
...
def decide_sell(code, holding, full_fund_data, big_market_latest, big_market_data, big_trend):
    ...
    target_nav = {
        ...
        'abs_stop_10_nav': round(cost_nav * (1 - 0.10), 4), # 新增：-10%止损价
        'short_drawdown_nav': np.nan # 留待最大回撤止损逻辑中计算
    }
    # -----------------------------------
    ...
    # --- 关键新增：T1 止盈规则（布林带中轨） ---
    # 策略目标：超卖反弹至均值区域即锁定部分利润 (布林中轨即MA20)
    # 仅在有盈利时才考虑此项，避免亏损时执行错误的减仓
    ...
    # ... (其余规则，最终的 'decision' 变量将被三要素决策覆盖，如果上面的高优先级决策没有触发)
    ...
    if len(full_fund_data) >= profit_lock_days:
        ...
        if (recent_rsi > 75).any() and bb_break:
            ...
            # 不返回，让它进入超规则和三要素，但如果有更高优先级决策则会被覆盖
    ...
    elif profit_rate < -10: sell_profit = '暂停定投' # 尽管-10%已前置，这里保留逻辑
    else: sell_profit = '持仓'
