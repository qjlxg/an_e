...
# --- 滚动年化收益率（正确版）---
def calculate_rolling_returns(nav, period_days):
    if len(nav) < period_days + 1:
        return np.nan
...
def calculate_metrics(df, start_date, end_date):
    ...
    for name, days in ROLLING_PERIODS.items():
        rolling[f'平均滚动年化收益率({name})'] = calculate_rolling_returns(nav, days)
...
def main():
    ...
    info_df = pd.DataFrame(info_list).rename(columns={
        'code': '基金代码_info', 'name': '基金简称', 'size': '资产规模',
    ...
    # 列顺序
    target_cols = [
        '基金代码', '基金简称', '资产规模', '基金类型', '最新日涨跌幅', '最新净值', '管理费率',
