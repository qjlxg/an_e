...
def calculate_metrics(df, start_date, end_date):
    ...
    # 使用累计净值进行计算
    cumulative_net_value = df['cumulative_net_value'].astype(float).replace(0, np.nan).dropna()
...
def main():
    ...
    for filename in file_list:
        ...
        try:
            ...
            df = df.rename(columns={'net_value': 'net_value', 'cumulative_net_value': 'cumulative_net_value'})
    ...
    summary_df['Sharpe_Ratio_Num'] = summary_df['Sharpe_Ratio'].str.replace('%', '').astype(float)
