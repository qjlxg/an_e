...
IndexInfo = indexinfo
# =======================================================
# 附加的合并逻辑块：请将此代码块完整粘贴到 info.py 文件的最末尾
# =======================================================

def get_codes_from_file(filename="C类.txt"):
    """
    读取文件，只提取有效的基金代码。
    """
    codes = []
    print(f"尝试从文件 {filename} 读取代码...")
    try:
        # 获取脚本所在目录，以便正确读取 C类.txt
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                code = line.strip()
                # 仅保留5到6位数字的代码
                if code.isdigit() and len(code) >= 5 and len(code) <= 6:
                    codes.append(code)
    except FileNotFoundError:
        print(f"错误：文件 {filename} 未找到。请确保它位于 info.py 同一目录。")
...
def run_analysis_from_file():
    """
    读取 C类.txt 中的代码，并逐个运行 fundinfo。
    将结果保存到 workflow_outputs/report.txt。
    """
    # 导入 os 和 datetime as dt (已在文件顶部导入，但此处再次声明以确保独立性)
    import datetime as dt
    import os
    try:
        # 从 xalpha.exceptions 捕获特定的错误 (已在文件顶部导入)
        from xalpha.exceptions import FundTypeError, TradeBehaviorError, ParserFailure
    except ImportError:
        # 文件顶部已导入，此警告通常不会触发，除非环境问题
        print("警告：xalpha.exceptions 导入失败，请检查依赖是否正确安装。")
        return
    ...
    output_lines = [f"--- 脚本运行报告 ({current_time}) ---"]
    output_lines.append(f"共发现 {len(codes)} 个有效基金代码。")
    
    if not codes:
        output_lines.append("未找到有效的基金代码。请检查 C类.txt 文件。")
        
    # 遍历基金代码
    for code in codes:
        output_lines.append(f"\n--- 正在处理基金代码: {code} ---")
        try:
            # 调用您文件中定义的 fundinfo 类
            # fetch=True 会尝试从本地加载，如果失败会联网获取
            fund = fundinfo(code, fetch=True, save=False) 
            output_lines.append(f"基金名称: {fund.name}")
            output_lines.append(f"基金代码: {fund.code}")
            
            # 获取最新净值日期
            latest_date = "N/A"
            if not fund.price.empty:
                # 确保 fund.price.iloc[-1]['date'] 是 datetime 对象
                latest_date = fund.price.iloc[-1]['date'].strftime('%Y-%m-%d')
            output_lines.append(f"价格表最后日期: {latest_date}")
            
        except (FundTypeError, TradeBehaviorError, ParserFailure) as e:
             # 捕获 xalpha 相关的错误
            output_lines.append(f"运行 fundinfo({code}) 失败 (xalpha 错误): {type(e).__name__}: {str(e)}")
        except Exception as e:
            # 捕获其他未知错误
            output_lines.append(f"运行 fundinfo({code}) 失败 (其他错误): {type(e).__name__}: {str(e)}")
    ...
    print(f"Script run complete. Output saved to {output_file}")
