...
def generate_report(results, timestamp_str):
    ...
    try:
        ...
        # 定义两个表格的头部和对齐分隔符
        # 表格 1 (6列): 排名, 基金代码, 最大回撤 (1M), 当日涨跌幅, RSI(14), 行动提示
        TABLE_1_HEADER = f"| 排名 | 基金代码 | 最大回撤 (1M) | **当日涨跌幅** | RSI(14) | 行动提示 |\n"
        TABLE_1_SEPARATOR = f"| :---: | :---: | :---: | :---: | :---: | :---: |\n" 
...
def generate_report_csv(results, current_date_str):
    ...
    if 'MA50/MA250' in df_results.columns and 'MA50/MA250趋势' in df_results.columns:
        ...
        # 检查 NaN 或数据不足的情况 (MA50/MA250 < 0.95 或者趋势向下)
        ma_ratio_condition = (df_results['MA50/MA250'].isna()) | (df_results['MA50/MA250'] < 0.95)
        trend_condition = (df_results['MA50/MA250趋势'] == '向下')
        data_na_condition = (df_results['MA50/MA250趋势'] == '数据不足') | df_results['MA50/MA250'].isna()
        
        df_results.loc[ma_ratio_condition | trend_condition, '趋势健康度'] = '不健康/向下'
    ...
    if 'RSI' in df_results.columns and '当日跌幅' in df_results.columns:
        df_results.loc[df_results['RSI'] <= STRONG_RSI_THRESHOLD_P2, '优先级'] = 'P2'
        df_results.loc[df_results['RSI'] <= EXTREME_RSI_THRESHOLD_P1, '优先级'] = 'P1'
        
        # P1A 和 P1B 的进一步细分
        df_results.loc[(df_results['优先级'] == 'P1') & (df_results['当日跌幅'] <= -MIN_DAILY_DROP_PERCENT), '优先级'] = 'P1A'
        df_results.loc[(df_results['优先级'] == 'P1') & (df_results['当日跌幅'] > -MIN_DAILY_DROP_PERCENT), '优先级'] = 'P1B'
    
    # 明确排序规则
    priority_order = ['P1A', 'P1B', 'P2', 'P3']
    ...
    if '最大回撤' in df_results.columns:
        ...
        df_results['最大回撤(%)'] = np.nan
        
    if '当日跌幅' in df_results.columns:
        df_results['当日跌幅(%)'] = df_results['当日跌幅'].apply(lambda x: f"{x:.2%}")
    else:
        df_results['当日跌幅(%)'] = np.nan

    if 'MA50/MA250' in df_results.columns:
        df_results['MA50/MA250(d2)'] = df_results['MA50/MA250'].apply(lambda x: f"{x:.2f}" if not pd.isna(x) else 'NaN')
    else:
    ...
    # 排除不在 df_results 中的列（例如，如果数据不足，某些技术指标列可能不存在）
    available_columns = [col for col in final_columns if col in df_results.columns]
...
def main():
    ...
    try:
        ...
        logging.info(f"📰 Markdown 报告已保存到 {report_file_md}")
